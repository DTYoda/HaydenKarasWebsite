...
export default function SkillsChart({ activeCategory}) {
    ...
    let [activeSector, setActiveSector] = useState(Object.keys(categories[activeCategory])[0]);
    ...
    return (<div className="w-full flex justify-normal flex-col p-0 m-0 h-full">
        <h1 className="sm:text-[4vw] text-[10vw] p-8 text-center text-7xl uppercase">{activeCategory}</h1>
        <div className="flex sm:flex-row flex-col flex-nowrap justify-between items-center">
            <div className="shrink-0 sm:w-[40vw] w-[80vw] sm:h-[40vw] h-[80vw] flex mx-auto overflow-hidden">
                <CircleChart skills={Object.keys(categories[activeCategory])} handleClick={handleSectorClick} sectorValue={sectorValue} currentCategory={activeCategory}/>
            </div>
            <div className="w-full sm:w-1/2 shrink grow">
                <h3 className="p-4 sm:text-[3.5vw] text-[6vw] font-bold">{activeSector}</h3>
                <p className="sm:text-[2vw] text-[4vw] w-full p-4 leading-loose">{categories[activeCategory][activeSector]}</p>
            </div>
