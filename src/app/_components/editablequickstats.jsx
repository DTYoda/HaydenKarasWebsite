...
export default function EditableQuickStats({ initialData }) {
  ...
  const { openEditModal, handleDelete, EditModalComponent } = useEditable(
    ...
    (result) => {
      if (result?.data) {
        setStats((prev) => {
          const exists = prev.some((stat) => stat.id === result.data.id);
          if (exists) {
            return prev.map((stat) =>
              stat.id === result.data.id ? result.data : stat
            );
          }
          return [...prev, result.data].sort(
                ...
                <>
                  ...
                  <DeleteButton
                    onClick={() => {
                      ...
                      setStats((prev) => prev.filter((item) => item.id !== stat.id));
                    }}
