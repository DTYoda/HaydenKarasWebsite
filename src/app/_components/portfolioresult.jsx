...
export default function PortfolioResult({ link, title, type, date, image }) {
  ...
  return (
    <a
      className="w-64 text-left group block glass rounded-2xl border border-orange-500/20 overflow-hidden hover-lift transition-all duration-300"
      href={"/portfolio/" + link}
    >
      ...
      <div className="w-64 flex flex-col p-4">
        <div className="font-bold text-lg mb-1 text-gray-200 group-hover:text-orange-500 transition-colors duration-300">
          {title}
          <span className="block max-w-0 group-hover:max-w-full transition-all duration-500 h-0.5 bg-orange-500 mt-1" />
        </div>
